...
object BostonCrimes {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val crimeFilename = "./src/files/crime.csv"
    val offenseCodesFilename = "./src/files/offense_codes.csv"
    val resultFolder = "./src/files/result"


    // Create Spark Session
    val spark : SparkSession = SparkSession
      .builder
      .appName(name = " BostonCrimes")
      .master(master = "local[*]")
      .getOrCreate()
    ...
    result.repartition(1).write.mode("OVERWRITE").csv(resultFolder)
